...
def string_to_bits(text):  # Convert a string into a list of bits
    array = list()
    for char in text:
...
def bits_to_string(array):  # Recreate the string from the bit array
...
def split(s, n):  # Split a list into parts of size "n"
...
def main(key, text, mode):
    ...
    keys = generatekeys()  # Generate all the keys
    text_blocks = split(text, 8)  # Split the text in blocks of 8 bytes so 64 bits
    result = list()
    for block in text_blocks:  # Loop over all the blocks of data
        block = string_to_bits(block)  # Convert the block in bit array
        block = permut(block, PI)  # Apply the initial permutation
        g, d = split(block, 32)  # g(LEFT), d(RIGHT)
    ...
    final_res = bits_to_tring(result)
    if mode == 1:
...
def substitute(d_e):  # Substitute bytes using SBOX
    subblocks = split(d_e, 6)  # Split bit array into sublist of 6 bits
    result = list()
...
def generatekeys():  # Algorithm that generates all the keys
    keys = []
    key = string_to_bit_array(password)
    key = permut(key, CP_1)  # Apply the initial permut on the key
    g, d = split(key, 28)  # Split it in to (g->LEFT),(d->RIGHT)
...
def addPadding():  # Add padding to the datas using PKCS5 spec.
    a = text
...
if __name__ == '__main__':
    ...
    text = "333"
    d = des()
    r = d.encrypt(key_string, text, padding=True)
    r2 = d.decrypt(key_string, r, padding=True)
    print("Ciphered: %r" % r)
    print("Deciphered: ", r2)